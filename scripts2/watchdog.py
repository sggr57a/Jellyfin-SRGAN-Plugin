...
def handle_play():
    ...
    try:
        ...
        if not input_file:
            ...
            logger.error("Expected: data['Item']['Path']")

            item_data = data.get("Item", {})
            item_keys = list(item_data.keys()) if isinstance(item_data, dict) else []

            logger.error(f"Received keys: Item={item_keys}")
            logger.error(f"Item type: {type(item_data).__name__}")
            logger.error(f"Item value: {item_data}")

            # Check if Item is missing or not a dict
            if not item_data:
                logger.error("")
                logger.error("╔════════════════════════════════════════════════════════════╗")
                logger.error("║  WEBHOOK TEMPLATE ERROR: Missing or Empty 'Item'           ║")
                logger.error("╚════════════════════════════════════════════════════════════╝")
                logger.error("")
                logger.error("The 'Item' section is missing or empty in webhook payload.")
                logger.error("")
                logger.error("Your template should have:")
                logger.error('  "Item": {')
                logger.error('    "Path": "{{Item.Path}}",')
                logger.error('    "Name": "{{Item.Name}}",')
                logger.error('    "Type": "{{Item.Type}}"')
                logger.error('  }')
                logger.error("")
                logger.error("FIX: Copy the correct template from WEBHOOK_SETUP.md")
                logger.error("")

                return jsonify({
                    "status": "error",
                    "message": "Missing or empty 'Item' in webhook payload",
                    "fix": "Check webhook template - see WEBHOOK_SETUP.md",
                    "received": data
                }), 400

            # Check if Path key is missing from Item
            if "Path" not in item_data:
                logger.error("")
                logger.error("╔════════════════════════════════════════════════════════════╗")
                logger.error("║  WEBHOOK TEMPLATE ERROR: Missing 'Path' Key                ║")
                logger.error("╚════════════════════════════════════════════════════════════╝")
                logger.error("")
                logger.error("The 'Item' section exists but has no 'Path' field.")
                logger.error("")
                logger.error(f"Item has these keys: {item_keys}")
                logger.error("")
                logger.error("Your template is missing the Path field!")
                logger.error("")
                logger.error("CORRECT template:")
                logger.error('  "Item": {')
                logger.error('    "Path": "{{Item.Path}}",    ← MISSING THIS!')
                logger.error('    "Name": "{{Item.Name}}",')
                logger.error('    "Type": "{{Item.Type}}"')
                logger.error('  }')
                logger.error("")
                logger.error("FIX: Add the Path field to your webhook template")
                logger.error("     Copy the complete template from WEBHOOK_SETUP.md")
                logger.error("")

                return jsonify({
                    "status": "error",
                    "message": "Missing 'Path' key in Item",
                    "fix": "Add Path field to webhook template",
                    "received_keys": item_keys,
                    "expected": "Path"
                }), 400
            ...
            if "Path" in item_data and item_data["Path"] == "":
                ...
                logger.error("╚════════════════════════════════════════════════════════════╝")
                logger.error("")
                logger.error("Template variables are EMPTY ({{Item.Path}} = '')")
                logger.error("")
                ...
                logger.error("")
                logger.error("  3. Template uses {{{}}} instead of {{}}")
                logger.error("     → Correct: {{Item.Path}}")
                logger.error("     → Wrong: {{{Item.Path}}}")
                logger.error("")
                logger.error("  4. Jellyfin Webhook plugin version issue")
                logger.error("     → Update to latest version")
                logger.error("")
                ...
                logger.error("  4. Verify template uses {{Item.Path}} (double braces)")
                logger.error("  5. Save and test by playing a video")
                logger.error("")
                logger.error("See: WEBHOOK_SETUP.md for complete configuration")
                logger.error("")

                return jsonify({
                    "status": "error",
                    "message": "Webhook template variables are empty",
                    "issue": "Item.Path is empty string - Jellyfin not filling template",
                    "fix": "Check Notification Type (Playback Start) and Item Type (Movie/Episode) in webhook config",
                    "documentation": "See WEBHOOK_SETUP.md"
                }), 400

            return jsonify({
                "status": "error",
                "message": "Missing Item.Path in payload"
            }), 400
